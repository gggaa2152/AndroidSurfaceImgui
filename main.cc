...
void UpdateFontHD(bool force = false) {
    ...
    float targetSize = std::clamp(baseSize, 8.0f, 600.0f); 
...
void DrawBoard() {
    ...
    if (isScalingBoard) {
        if (ImGui::IsMouseDown(0)) {
            ...
            float baseW_B = (6.5f * 1.73205f + 1.0f) * 38.0f * g_boardScale * g_autoScale;
            g_boardManualScale = std::max(curW / baseW_B, 0.1f); 
