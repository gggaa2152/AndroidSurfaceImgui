...
// 增加一个标记，用于在 BeginFrame 之前安全更新字体
...
void UpdateFontHD(bool force = false) {
    ...
    // 关键修正：此时必须确保 BeginFrame 尚未调用或 Atlas 未被锁定
        ...
        if (!g_menuCollapsed) {
            ...
            if (isScalingMenu) { 
                if (ImGui::IsMouseDown(0)) {
                    ...
                    isScalingMenu = false; 
                    g_needUpdateFontSafe = true; // 标记需要更新字体
                    SaveConfig();
                } 
            }
...
int main() {
    ...
    while (running) {
        // --- 核心修复：更新字体必须在 BeginFrame 之前，此时 Atlas 未锁定 ---
