...
bool is_process_really_alive(pid_t pid) {
    ...
    if (!alive) {
        char path[128]; snprintf(path, 128, "/proc/%d/cmdline", pid);
        alive = (access(path, F_OK) == 0);
    }
    if (alive) { fail_count = 0; return true; } 
    else { fail_count++; return fail_count < 5; }
}
...
int perform_injection(pid_t pid, const char* drop_path) {
    ...
    auto safe_call = [&](uintptr_t func, long* params, int num) -> long {
        ...
        int status;
        while (true) {
            ptrace(PTRACE_CONT, pid, NULL, 0); waitpid(pid, &status, WUNTRACED);
    ...
    iov.iov_base = &saved; ptrace(PTRACE_SETREGSET, pid, (void*)NT_PRSTATUS, &iov);
...
int main(int argc, char** argv) {
    LOGI("=============================================");
    LOGI("   JKHelper Daemon - 完美融合原版UI与防闪退");
    LOGI("=============================================");
