...
// =================================================================
// 6. 菜单 UI (改进版：使用 InvisibleButton 处理标题栏和缩放手柄)
// =================================================================
...
void DrawMenu() {
    static bool isScalingMenu = false; // 是否正在缩放菜单
    static float startMS = 1.0f;       // 缩放开始时的缩放系数
    static ImVec2 startMP;              // 缩放开始时的鼠标位置

    ImGuiIO& io = ImGui::GetIO();
    float baseW = 320.0f * g_autoScale; // 基础宽度（已乘自动缩放）
    float baseH = 500.0f * g_autoScale; // 基础高度
    float currentW = baseW * g_scale;   // 当前宽度
    float currentH = g_menuCollapsed ? ImGui::GetFrameHeight() : (baseH * g_scale); // 折叠时只显示标题栏高度
...
        float hSz = 50.0f * g_autoScale * g_scale; // 手柄大小
...
        ImGui::InvisibleButton("##scaleHandle", handleRect.GetSize());
